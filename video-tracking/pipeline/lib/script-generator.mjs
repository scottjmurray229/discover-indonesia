...
export async function generateScript(dest, { apiKey, outputDir, force = false }) {
  ...
  const prompt = `Write a 60-word narration script for a 30-second YouTube Short about ${dest.title}, Indonesia.
...
3. CTA (1 sentence) — direct viewers to discoverindonesia.info and subscribe
...
IMPORTANT: Write ALL numbers and dollar amounts as full words for text-to-speech. Say "thirty dollars" not "$30". Say "one hundred fifty dollars" not "$150". Say "discover indonesia dot info" not "discoverindonesia.info".`;
