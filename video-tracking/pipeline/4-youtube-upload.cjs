 ...
 *   2. Create project "Discover Indonesia"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Indonesia Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'indonesia'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Indonesia ${year}`,
      description: `${title}\n\nFull guide: https://discoverindonesia.info/${slug}/`,
      tags: [`Indonesia ${slug}`, 'Indonesia travel', `Indonesia ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Indonesia ✈️ #shorts`,
      description: `${title} — Full guide: https://discoverindonesia.info/destinations/${slug}/`,
      tags: [`${title} Indonesia`, 'Indonesia travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Indonesia — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Indonesia for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoverindonesia.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoverindonesia.info/companion/

#Indonesia #${title.replace(/\s+/g, '')} #IndonesiaTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  ...
  if (dest?.region) tags.push(`${dest.region} Indonesia`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER INDONESIA              ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
