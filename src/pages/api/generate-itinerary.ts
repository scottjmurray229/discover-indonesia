...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['bali', 'gili-islands', 'lombok'],
    'island': ['gili-islands', 'raja-ampat', 'nusa-penida'],
    'snorkel': ['nusa-penida', 'gili-islands', 'komodo'],
    'dive': ['raja-ampat', 'komodo', 'flores'],
    'diving': ['raja-ampat', 'komodo', 'nusa-penida'],
    'surf': ['bali', 'lombok'],
    'culture': ['yogyakarta', 'ubud', 'bali'],
    'history': ['yogyakarta', 'jakarta', 'bandung'],
    'food': ['jakarta', 'medan', 'bandung'],
    'relax': ['ubud', 'gili-islands', 'nusa-penida'],
    'adventure': ['raja-ampat', 'komodo', 'flores'],
    'family': ['bali', 'yogyakarta', 'lombok'],
    'honeymoon': ['ubud', 'gili-islands', 'raja-ampat'],
    'romantic': ['ubud', 'nusa-penida', 'gili-islands'],
    'budget': ['yogyakarta', 'lombok', 'malang'],
    'luxury': ['ubud', 'bali', 'raja-ampat'],
    'java': ['yogyakarta', 'jakarta', 'surabaya'],
    'sumatra': ['lake-toba', 'medan'],
  };
...
function buildSystemPrompt(): string {
  return `You are the Discover Indonesia AI Trip Planner. You generate detailed day-by-day itineraries for Indonesia travel.

RULES:
- All prices in BOTH IDR (Rp) and USD ($). Use rate: $1 = Rp16,000
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-bali"
...
{
  "title": "string — trip title like '7-Day Bali & Java Adventure'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "idr": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Bali'",
      "destination": "string — slug like 'bali'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Tegallalang Rice Terraces', 'Borobudur Temple', 'Manta Point Nusa Penida'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Indonesia trip:\n${parts.join('\n')}`;
